...
EOF
  ...
  git init
  git config user.name "charblus"
  git config user.email "charblus7582@gmail.com"
  git add .
  git commit -m "Update Blog By TravisCI With Build $TRAVIS_BUILD_NUMBER"
  # Github Pages
  git push --force --quiet "https://${GH_TOKEN}@${GH_REF}" master:master
  # Gitee Pages
  git push --force --quiet "https://charblus:${GITEE_TOKEN}@${GITEE_REF}" master:master
  ...
  # Gitee Pages
  git push --quiet "https://charblus:${GITEE_TOKEN}@${GITEE_REF}" master:master --tags
}
